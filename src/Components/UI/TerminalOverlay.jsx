...
const TerminalOverlay = ({ isOpen, onClose }) => {
  ...
  const [logs, setLogs] = useState([
    { src: 'SYS', msg: 'Initializing Neural Link v3.9...' },
  ...
  const handleCommand = async (e) => {
    ...
    // 1. Try Gemini AI (Using 'gemini-pro' for broader compatibility)
    try {
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${GEMINI_API_KEY}`,
        {
          ...
          body: JSON.stringify({
            ...
            // System instructions are handled differently in older/standard models, 
            // so we prepend it to the user query context for safety
            contents: [{
      ...
      // Show specific error in terminal to help debug
      setLogs(prev => [...prev, { 
        src: 'ERR', 
        msg: `[!] Connection Failed: ${err.message.substring(0, 50)}...` 
      }]);
    } finally {
      ...
      <div className="w-full max-w-3xl h-[600px] bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl flex flex-col overflow-hidden relative">
        ...
        <div className="h-10 border-b border-zinc-800 flex items-center justify-between px-4 bg-zinc-900/50">
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="text-zinc-400">NEURAL_LINK_V3.9</span>
