...
const SkillTag = ({ name, icon: Icon, isDark }) => (
  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-medium transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${
    isDark 
      ? 'bg-zinc-900/50 border-zinc-800 text-zinc-300 hover:border-indigo-500/50 hover:bg-zinc-800 hover:text-indigo-400 hover:shadow-indigo-500/20' 
      : 'bg-white border-zinc-200 text-zinc-700 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-600 hover:shadow-indigo-500/10'
  }`}>
...
const CategoryCard = ({ title, icon: Icon, skills, isDark, delay }) => (
  <Reveal delay={delay}>
    <div className={`relative h-full p-6 rounded-2xl border backdrop-blur-sm transition-all duration-500 hover:shadow-xl ${
      isDark 
      ...
      <div className="absolute -top-4 -right-4 w-24 h-24 bg-indigo-500/10 rounded-full blur-2xl pointer-events-none"></div>
      
      <div className="flex items-center gap-3 mb-6 relative z-10">
        <div className={`p-2 rounded-xl ${isDark ? 'bg-zinc-800 text-indigo-400' : 'bg-indigo-100 text-indigo-600'}`}>
...
const TechStack = ({ resumeData, isDark }) => {
  ...
  return (
    <section id="techstack" className={`px-6 py-24 md:py-32 antialiased relative overflow-hidden ${isDark ? 'bg-zinc-950' : 'bg-gray-50'}`}>
      ...
      <div className="absolute top-1/4 -left-64 w-96 h-96 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 -right-64 w-96 h-96 bg-purple-500/5 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <Reveal>
          <div className="mb-16">
            ...
            <p className={`text-sm md:text-base max-w-2xl ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
              A comprehensive toolkit focused on engineering intelligent AI/ML systems, supported by a robust foundation in full-stack development, cloud architecture, and DevOps principles.
            </p>
          </div>
        </Reveal>
